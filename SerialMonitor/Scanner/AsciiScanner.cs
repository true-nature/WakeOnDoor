...
namespace SerialMonitor.Scanner
{
    /// ascii style
    /// PAL
    /// :80000000 AE 0045 82018032 01 80 81 03 11300802 0B4A 11300102 04C4 00000001 81 F82D
    /// ADXL345
    ...
    {
        private static readonly Regex PalAppRegex = new Regex(@"^:(?<Rptr>[\dA-F]{8})(?<Lqi>[\dA-F]{2})(?<Ct>[\dA-F]{4})(?<Sid>[\dA-F]{8})(?<Id>[\dA-F]{2})(?<Pkt>[\dA-F]{2})(?<PalId>[\dA-F]{2})(?<Nsns>03)11300802(?<Batt>[\dA-F]{4})11300102(?<Adc1>[\dA-F]{4})00000001(?<Mag>[\dA-F]{2})(?<CHK1>[\dA-F]{2})(?<CHK2>[\dA-F]{2})$");
        ...
        private static readonly Regex[] regexs = { PalAppRegex, ADXL345Regex, ButtonRegex };
                ...
                foreach (var r in regexs)
                {
                    ...
                    if (m.Success)
                    {
                        ...
                        if (info.Valid &&
                            ...
                            || (info.Pkt == PacketId.PAL && (info.Mag == 0))   // 磁気が遠ざかった
